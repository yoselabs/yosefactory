...
- [ ] 1.1 Add `--queue` and `--workspace` (optional `Path`, default `None`) to `main`'s argparse
      surface in `src/yosefactory/runtime/loop.py`.
- [ ] 1.2 Add `--test-command` (optional `str`, `shlex.split` when given, default
      `verify.DEFAULT_TEST_COMMAND` unchanged) and thread it through to `run_loop`'s
      `test_command` kwarg.
- [ ] 1.3 Add `--cost-ceiling-usd` (optional `float`, default `None`) and wire it into the
...
- [ ] 2.1 Replace `places = Places.local(repo)` with resolution: `queue = (args.queue or
      args.repo).resolve()`, `workspace = (args.workspace or args.repo).resolve()`.
- [ ] 2.2 When `queue == workspace`, build `Places` exactly as `Places.local` does today (same
...
- [ ] 2.3 When `queue != workspace`, build `Places` with `ledger=queue/RUNS`,
      `queue_lock=queue/LOCK`, `workspace_lock=workspace/".git"/"yosefactory-turn.lock"`.
- [ ] 2.4 Keep the existing `unattended` publish-boolean wiring (`--publish` sets both
      `publish_queue`/`publish_workspace`) unchanged and applying to whichever `Places` was built.

## 3. Spec delta

- [ ] 3.1 `specs/containerized-loop/cross-repo-invocation/spec.md` — already drafted; confirm it
...
- [ ] 4.1 Unit: omitting `--queue`/`--workspace` produces a `Places` identical to
...
- [ ] 4.2 Unit: `--cost-ceiling-usd` reaches `Guardrails.cost_ceiling_usd`; `--spend-ceiling-usd`
...
- [ ] 4.3 Unit: `--test-command "make check"` reaches `run_loop`'s `test_command` kwarg as
      `("make", "check")`; omitted, it is `DEFAULT_TEST_COMMAND`.
- [ ] 4.4 **The end-to-end receipt this change exists for**: two temporary git repositories (queue,
...
- [ ] 4.5 Regression: every existing `test_loop.py` invocation that omits the new flags still
      passes unmodified — the collapsed single-repo case is untouched.

## 5. Verify by construction against the driver's inventory

- [ ] 5.1 Walk `design.md`'s driver-inventory table line by line against the shipped flags; for
...
- [ ] 5.2 `make check` green.

## 6. ADR

- [ ] 6.1 Write `decisions/00NN-*.md` (next available number) recording: the two-flag collapse
