...
- [x] 1.1 Add `gate_rejected` to `backlog.ITEM.rules`: `Rule(frozenset({"doing"}), None, required=(("report",), ("attempt",)))`.
- [x] 1.2 Add `backlog.context(item: FoldedLog) -> dict[str, Any]`, folding the four D030 sources
      (`gate_rejected`, `unblocked.resolution.answer`, `failed`, `released`/`reclaimed`),
      last-one-wins per source, same pattern as `frame()`.
- [x] 1.3 Update `VOCABULARY_SPEC`-mirrored table (`openspec/specs/backlog-item-format/spec.md`)
      only via this change's spec delta — do not hand-edit the promoted spec.

## 2. Runtime — gate rejection reaches the item

- [x] 2.1 In `_dispose`'s `done` branch (`runtime/turn.py`), on `not gate.passed`: append
      `gate_rejected` (`report=gate.report()`, `attempt` read from
      `backlog.lease(backlog.load(item_path))`) to `item_path` before `return failed(...)`.
- [x] 2.2 Confirm the append lands in the same commit as the existing `failed(...)` turn record —
      `item_path` is already in `touched`/`paths` for that branch; no new commit call needed.
- [x] 2.3 Confirm `_poison_if_exhausted` is not reachable from this path (it is not — the branch
...
- [x] 3.1 In `apply_answers()` (`runtime/turn.py`), read the closing question record's `answer`
...
- [x] 3.2 No change to `question.py` — the canonical answer stays in the question log; the item's
      copy is written, never re-read to make a second decision.

## 4. Executor seam

- [x] 4.1 `Executor.__call__` (`runtime/turn.py`): add keyword-only
      `context: Mapping[str, Any] | None = None`, positioned after `frame`.
- [x] 4.2 `take_turn`'s acting branch: compute `context = backlog.context(backlog.load(item_path))`
      beside the existing `frame = backlog.frame(...)` line; pass both to `executor(...)`.
- [x] 4.3 `take_turn`'s planning branch: pass no context (planning has no item to fold one from).
- [x] 4.4 `executor/claude.py`'s `render()`: accept `context: Mapping[str, Any] | None = None`;
...
- [x] 4.5 `executor/claude.py`'s `run()`: thread `context` through to `render()`.

## 5. Tests

- [x] 5.1 `tests/protocol/test_backlog_fold.py`: `gate_rejected` legal from `doing`, illegal
      elsewhere; does not change state; repeated `gate_rejected` does not poison.
- [x] 5.2 `tests/protocol/test_backlog_fold.py` (or new): `backlog.context()` — each of the four
...
- [x] 5.3 `tests/protocol/test_turn.py` / `tests/runtime/test_turn_cycle.py`: a rejected `done`
...
- [x] 5.4 A test exercising `apply_answers()` end to end: an `answered` question's text appears in
      the resulting `unblocked.resolution.answer`, and in the next turn's folded `context`.
- [x] 5.5 `render()` unit test: context block appears between frame lines and invocation lines when
