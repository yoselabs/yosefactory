...
signature-conformance check performed with `inspect.signature` — SHALL NOT be gated on the
binary's presence or version, regardless of which file it is defined alongside.
...
do with what it actually checked.
